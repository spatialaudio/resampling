...
%     % Interpolationsverfahren
    ...
    t4= 0:dt/4:t(end);
    y4 = sin(2*pi*f(idx)*t4); %interp(y,4);
    ...
    t8= 0:dt/8:t(end);
    y8 = sin(2*pi*f(idx)*t8); % interp(y,8);
...
hold on
semilogx(f, mag2db(err_lagrange_2_u));
hold on
...
xlabel('Frequenz [Hz]');
ylabel('Fehler [dB]');
legend('ZoH', 'ZoH, X = 4', 'ZoH, X = 8', 'Bandlimited X = 5',  'Lagrange N = 1', 'Lagrange N = 1, X = 4','Lagrange N = 3',  'Lagrange N = 3, X = 4',   'Location', 'Southwest')
